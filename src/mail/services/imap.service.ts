...
export class ImapService implements OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit() {
    this.logger.log('🔄 Initializing IMAP Mailboxes...');
    const boxes = await this.prisma.mailbox.findMany();
  ...
  private async startMailbox(box: any) {
    const { imap_host, imap_port, imap_password, email_address, is_ssl } = box;
    ...
    client.on('error', (err) => {
      this.logger.error(`❌ IMAP Error (${email_address}): ${err.message}`);
      this.reconnect(box);
    });

    client.on('close', () => {
      this.logger.warn(`⚠️ IMAP Closed → ${email_address}`);
      this.reconnect(box);
    });

    try {
      await client.connect();
      this.logger.log(`✅ IMAP Connected → ${email_address}`);

      this.clients.set(box.id, client);
      await this.syncInbox(box.id);
    } catch (err) {
      this.logger.error(`❌ IMAP Login Failed → ${email_address}`);
      this.logger.error(err);
      this.reconnect(box);
    }
  }
  ...
  private reconnect(box: any) {
    setTimeout(() => this.startMailbox(box), 20000);
  }
